...
import game.network.*;
import game.network.ScoreServer.Score;
...
class Scene extends Sprite {
	...
	private function highScoresLoaded(scores:Array<Score>) {
		// массив новой таблицы результатов
		var newScores:Array<Score> = [];
		// позиция счета игрока в таблице
		var myScorePos = -1;
		// отсортируем очки по убыванию
		scores.sort(function(a:Score, b:Score):Int {
		...
		for(s in scores) if(s.BESTSCORE > gameplay.score) newScores.push(s);
		// добавим результат игрока
		newScores.push({ USERNAME: AuthMenu.USERNAME, BESTSCORE : gameplay.score });
		...
		ScoreServer.setHighScores(gameplay.score);
	}
}